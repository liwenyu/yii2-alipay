...
class Submit
{
    ...
    public function buildRequestPara($para_temp, $aliapy_config)
    {
        ...
        //生成签名结果
        $mysign = $core->buildMysign($para_sort, trim($aliapy_config['key']), strtoupper(trim($aliapy_config['sign_type'])));
        //签名结果与签名方式加入请求提交参数组中
    ...
    public function sendPostInfo($para_temp, $gateway, $aliapy_config)
    {
        ...
        //远程获取数据
        $core = new Core();
        $xml_data = $core->getHttpResponse($url, trim(strtolower($aliapy_config['input_charset'])));
        //解析XML
        $doc = new DOMDocument();
